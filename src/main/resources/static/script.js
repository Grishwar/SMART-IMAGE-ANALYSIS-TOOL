...
function parseReport(report){
    ...
    // Split by separator — each block starts with File:
    // We join consecutive pieces to make sure RF data at end is included
    const rawBlocks = report.split("============================================");

    // Rebuild proper blocks: each block = from one "File:" to next "File:"
    let blocks = [];
    let current = "";
    rawBlocks.forEach(piece => {
        if(piece.includes("File:") && current.includes("File:")){
            blocks.push(current);
            current = piece;
        } else {
            current += piece;
    ...
    if(current.includes("File:")) blocks.push(current);
    ...
    blocks.forEach(block=>{
       ...
       block.split("\n").forEach(line => {
           line = line.trim();
           if(line.startsWith("RF Verdict:"))
               rfVerdict = line.replace("RF Verdict:","").trim();
           if(line.startsWith("RF Tampered Confidence:"))
               rfTampered = line.replace("RF Tampered Confidence:","").replace("%","").trim();
           if(line.startsWith("RF Genuine Confidence:"))
               rfGenuine = line.replace("RF Genuine Confidence:","").replace("%","").trim();
           if(line.startsWith("SVM Verdict:"))
               svmVerdict = line.replace("SVM Verdict:","").trim();
           if(line.startsWith("SVM Tampered Confidence:"))
               svmTampered = line.replace("SVM Tampered Confidence:","").replace("%","").trim();
           if(line.startsWith("SVM Genuine Confidence:"))
               svmGenuine = line.replace("SVM Genuine Confidence:","").replace("%","").trim();
       });
        ...
        <td style="text-align:center">
            <span style="font-weight:bold;font-size:13px;color:${parseFloat(rfTampered)>=70?'#ff4444':parseFloat(rfTampered)>=40?'#ffaa00':'#00ff88'}">
                ${rfVerdict}
            </span><br>
            <small style="color:#aaa">Tampered: ${rfTampered}%</small><br>
            <small style="color:#aaa">Genuine: ${rfGenuine}%</small>
        </td>
        <td style="text-align:center">
            <span style="font-weight:bold;font-size:13px;color:${parseFloat(svmTampered)>=70?'#ff4444':parseFloat(svmTampered)>=40?'#ffaa00':'#00ff88'}">
                ${svmVerdict}
            </span><br>
            <small style="color:#aaa">Tampered: ${svmTampered}%</small><br>
            <small style="color:#aaa">Genuine: ${svmGenuine}%</small>
        </td>
        <td>
    ...
    // Show K-Means cluster results
    if(report.includes("K-MEANS CLUSTER ANALYSIS")){
        let kmeansSection = report.split("=========== K-MEANS CLUSTER ANALYSIS ===========")[1];
        if(kmeansSection){
            let kmeansDiv = document.getElementById("kmeansResult");
            if(kmeansDiv){
                kmeansDiv.style.display="block";
                kmeansDiv.innerHTML = "<h3 style='color:#00ffd0'>K-Means Cluster Analysis</h3><pre style='color:white;font-size:13px'>" + kmeansSection + "</pre>";
            }
        }
    }
...
function formatMetadata(text){
    ...
    "RF Verdict:","RF Tampered Confidence:","RF Genuine Confidence:","Feature Contributions"];
    let out="";
    text.split("\n").forEach(l=>{
